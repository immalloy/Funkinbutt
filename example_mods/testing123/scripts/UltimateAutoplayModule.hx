...
import funkin.play.scoring.Scoring.ScoringSystem;
...
class UltimateAutoplayModule extends Module
{
	...
	public static var isBotActive = true;
	...
	override function onSongLoaded(event:SongLoadScriptEvent)
	{
		...
		UltimateAutoplayModule.isBotActive = true;
	}
		...
		function onNoteHit(event)
		{
			...
			var hitEvent:HitNoteScriptEvent = Std.isOfType(event, HitNoteScriptEvent) ? cast event : null;
			if (hitEvent == null || hitEvent.note == null)
				return;

			if (!hitEvent.note.noteData.getMustHitNote())
				return;

			var scoringSystem = getActiveScoringSystem();
			hitEvent.hitDiff = 0;
			hitEvent.judgement = "sick";
			hitEvent.isComboBreak = false;
			hitEvent.healthChange = Constants.HEALTH_SICK_BONUS;
			hitEvent.doesNotesplash = true;
			hitEvent.score = getPerfectScore(scoringSystem, 0);
		}
	...
	override function onUpdate(event:ScriptEvent):Void
	{
		...
		for (note in playerStrumline.getNotesMayHit())
		{
			...
			if (currentSongTime >= note.strumTime)
			{
				...
				var score = getPerfectScore(scoringSystem, perfectHitDiff);
				var daRating = "sick";
				var healthBonus = Constants.HEALTH_SICK_BONUS;
				var comboCount = note.scoreable ? Highscore.tallies.combo + 1 : Highscore.tallies.combo;

				var hitEvent = new HitNoteScriptEvent(note, healthBonus, score, daRating, false, comboCount, perfectHitDiff, true);
	...
	// Mirror the state's scoring system so perfect hits award the proper sick score value.
	private function getActiveScoringSystem():Null<ScoringSystem>
	{
		var state = PlayState.instance;
		if (state != null && ReflectUtil.hasField(state, "scoringSystem"))
		{
			var scoringField = ReflectUtil.field(state, "scoringSystem");
			if (scoringField != null)
			{
				var scoringString = Std.string(scoringField);
				if (scoringString != null && scoringString.length > 0)
				{
					return cast scoringString;
				}
			}
		}
	...
	private inline function getPerfectScore(scoringSystem:Null<ScoringSystem>, perfectHitDiff:Float):Int
	{
		return scoringSystem != null
			? Scoring.scoreNote(perfectHitDiff, cast scoringSystem)
			: Scoring.scoreNote(perfectHitDiff);
	}
}
